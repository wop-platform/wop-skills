...
- `skills/wop-cli/scripts/requirements.lock`：`wop-python-sdk @ git+https://github.com/wop-platform/wop-python-sdk`（pypi 未发布，git 引用；WOP_SDK_PATH env 支持本地覆盖）+ `tests/fixtures/crypto-vectors.json` 字节副本（MD5 断言锚）
